...
AWG_KERNEL_VERSION = "3.1.20260906"
...
# OpenWrt package maintainer. This is the maintainer of the OpenWrt packages,
# not the upstream Amnezia project authors.
AWG_MAINTAINER = f"{PROJECT_AUTHOR} <{PROJECT_EMAIL}>"

# Package license metadata comes from the actual generated package/upstream,
# not merely from the WireGuard OpenWrt package used as a generation template.
AWG_TOOLS_LICENSE = "GPL-2.0-only"
AWG_TOOLS_LICENSE_FILES = "COPYING"
AWG_KERNEL_LICENSE = "GPL-2.0-only"
AWG_KERNEL_LICENSE_FILES = "COPYING"
AWG_LUCI_LICENSE = "Apache-2.0"
...
def github_repo_url(repo: str) -> str:
    """Return a GitHub repository URL suitable for HTTP archive downloads."""
...
def github_codeload_base(repo: str) -> str:
    repo = github_repo_url(repo)
    prefix = "https://github.com/"

    if not repo.startswith(prefix):
        raise RuntimeError(f"not a GitHub repository URL: {repo}")

    repo_path = repo[len(prefix) :]
    return f"https://codeload.github.com/{repo_path}/tar.gz/refs/tags/"


def github_tag_archive_url(repo: str, version: str) -> str:
    return f"{github_codeload_base(repo)}v{version}?"


def sha256_url(url: str) -> str:
    """Download an upstream archive and return its SHA-256 digest."""

    print("Calculating archive SHA-256:")
    print(f"  url: {url}")
    ...
    if not re.fullmatch(r"[0-9a-f]{64}", source_hash):
        raise RuntimeError(f"invalid archive SHA-256: {source_hash!r}")

    print(f"  PKG_HASH: {source_hash}")
    return source_hash
...
def update_tools_makefile(path: Path) -> None:
    source_hash = sha256_url(github_tag_archive_url(AWG_TOOLS_REPO, AWG_TOOLS_VERSION))
    source_url = f"{github_codeload_base(AWG_TOOLS_REPO)}v$(PKG_VERSION)?"
    ...
    # The 25.12 WireGuard template uses the deprecated SPDX alias GPL-2.0.
    # Require that exact baseline value, then replace it with the license
    # metadata of the actual generated AmneziaWG tools package.
    text = sub_regex_once(
        text,
        r"^PKG_LICENSE:=GPL-2\.0$",
        f"PKG_LICENSE:={AWG_TOOLS_LICENSE}",
        path=path,
        label="tools package license",
        flags=re.MULTILINE,
    )

    text = sub_regex_once(
        text,
        r"^PKG_LICENSE_FILES:=COPYING$",
        (
            f"PKG_LICENSE_FILES:={AWG_TOOLS_LICENSE_FILES}\n"
            f"PKG_MAINTAINER:={AWG_MAINTAINER}"
        ),
        path=path,
        label="tools license files / maintainer",
        flags=re.MULTILINE,
    )

    # The older WireGuard template carries MAINTAINER inside define Package/.
    # Current OpenWrt metadata derives package MAINTAINER from PKG_MAINTAINER.
    text = sub_regex_once(
        text,
        r"^[ \t]*MAINTAINER:=.*\n",
        "",
        path=path,
        label="remove legacy tools MAINTAINER",
        flags=re.MULTILINE,
    )
    ...
    for metadata_line in expected_metadata:
        if text.count(metadata_line) != 1:
            raise RuntimeError(
                f"expected exactly one {metadata_line!r} marker in {path}"
            )

    if re.search(r"^[ \t]*MAINTAINER:=", text, flags=re.MULTILINE):
        raise RuntimeError(f"legacy tools MAINTAINER marker still present in {path}")
...
def fix_tools_description(path: Path) -> None:
    text = read_text(path)
    old = (
    ...
    text = replace_literal_once(
        ...
        new,
        path=path,
        label="tools package description",
    )
    write_text(path, text)
...
def update_luci_makefile(path: Path) -> None:
    ...
    # luci-proto-amneziawg is derived from the Apache-2.0 LuCI WireGuard code.
    # Keep the license explicit and fail if the selected LuCI baseline changes.
    text = sub_regex_once(
        text,
        r"^PKG_LICENSE:=Apache-2\.0$",
        f"PKG_LICENSE:={AWG_LUCI_LICENSE}",
        path=path,
        label="LuCI package license",
        flags=re.MULTILINE,
    )
    ...
    license_line = f"PKG_LICENSE:={AWG_LUCI_LICENSE}"
    if text.count(license_line) != 1:
        raise RuntimeError(f"PKG_LICENSE marker not present exactly once in {path}")
...
def gen_tools_config_gets() -> str:
    ...
    for opt in AWG_OPTS:
        if opt.kind == "bool":
            lines.append(f'\tconfig_get_bool {opt.uci} "${{config}}" "{opt.uci}" ""')
        else:
...
def gen_kmod_makefile() -> str:
    source_hash = sha256_url(
        github_tag_archive_url(AWG_KERNEL_REPO, AWG_KERNEL_VERSION)
    )
    source_url = f"{github_codeload_base(AWG_KERNEL_REPO)}v$(PKG_VERSION)?"
    ...
    lines = [
        ...
        "",
        f"PKG_LICENSE:={AWG_KERNEL_LICENSE}",
        f"PKG_LICENSE_FILES:={AWG_KERNEL_LICENSE_FILES}",
        f"PKG_MAINTAINER:={AWG_MAINTAINER}",
        "",
        ...
        f"  DEPENDS:= {continuation}",
        f"    +kmod-udptunnel4 {continuation}",
        f"    +IPV6:kmod-udptunnel6 {continuation}",
        f"    +kmod-crypto-lib-chacha20poly1305 {continuation}",
...
def update_tools(*, stage: str) -> None:
    ...
    fix_tools_description(AWG_TOOLS_DIR / "Makefile")
